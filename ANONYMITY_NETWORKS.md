...
`--proxy` which tells `scalad` the ip/port of a socks proxy provided by a
...
An ISP can passively monitor `scalad` connections from a node and observe when
...
only using I2P for broadcasting Scala transactions, the total aggregate of
I2P data would also leak information.

#### Mitigation

There is no current mitigation for the user right now. This attack is fairly
sophisticated, and likely requires support from the internet host of a Scala
user.
