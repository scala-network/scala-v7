...
### On Linux for Android (using docker):
        ...
        docker build -f utils/build_scripts/android32.Dockerfile -t stellite-android .
        # Build image (for ARM 64-bit)
        docker build -f utils/build_scripts/android64.Dockerfile -t stellite-android .
        # Create container
        docker create -it --name stellite-android stellite-android bash
        # Get binaries
        docker cp stellite-android:/src/build/release/bin .
...
Using depends might also be easier to compile stellite on windows than using msys. Activate windows subsystem for linux (for example ubuntu) install the apt build-essentials and follow the depends steps as depicted above.
...
Configure Stellite with the -D SANITIZE=ON cmake flag, eg:

    cd build/debug && cmake -D SANITIZE=ON -D CMAKE_BUILD_TYPE=Debug ../..

You can then run the stellite tools normally. Performance will typically halve.

* valgrind

Install valgrind and run as `valgrind /path/to/stellited`. It will be very slow.
