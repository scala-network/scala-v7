...
void initCache(const char (&key)[N]) {
	...
	defyx_init_cache(cache, key, N - 1);
	if (vm != nullptr)
		defyx_vm_set_cache(vm, cache);
}
...
void calcStringHash(const char(&key)[K], const char(&input)[H], void* output) {
	initCache(key);
	assert(vm != nullptr);
	defyx_calculate_hash(vm, input, H - 1, output);
}
...
void calcHexHash(const char(&key)[K], const char(&hex)[H], void* output) {
	...
	defyx_calculate_hash(vm, input, sizeof(input), output);
}
...
int main() {
	...
	//std::cout << "Allocating defyx_cache..." << std::endl;
	cache = defyx_alloc_cache(RANDOMX_FLAG_DEFAULT);

	runTest("Cache initialization", RANDOMX_ARGON_ITERATIONS == 3 && RANDOMX_ARGON_LANES == 1 && RANDOMX_ARGON_MEMORY == 262144 && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"),	[]() {
	...
	runTest("SuperscalarHash generator", RANDOMX_SUPERSCALAR_LATENCY == 170, []() {
		char sprogHash[32];
		defyx::SuperscalarProgram sprog;
		...
		for (int i = 0; i < 10; ++i) {
			defyx::generateSuperscalar(sprog, gen);
			blake2b(sprogHash, sizeof(sprogHash), &sprog.programBuffer, sizeof(defyx::Instruction) * sprog.getSize(), nullptr, 0);
			assert(equalsHex(sprogHash, superscalarReferences[i]));
		}
	});

	runTest("defyx_reciprocal", true, []() {
		assert(defyx_reciprocal(3) == 12297829382473034410U);
		assert(defyx_reciprocal(13) == 11351842506898185609U);
		assert(defyx_reciprocal(33) == 17887751829051686415U);
		assert(defyx_reciprocal(65537) == 18446462603027742720U);
		assert(defyx_reciprocal(15000001) == 10316166306300415204U);
		assert(defyx_reciprocal(3845182035) == 10302264209224146340U);
		assert(defyx_reciprocal(0xffffffff) == 9223372039002259456U);
	});

	runTest("defyx_reciprocal_fast", RANDOMX_HAVE_FAST_RECIPROCAL, []() {
		assert(defyx_reciprocal_fast(3) == 12297829382473034410U);
		assert(defyx_reciprocal_fast(13) == 11351842506898185609U);
		assert(defyx_reciprocal_fast(33) == 17887751829051686415U);
		assert(defyx_reciprocal_fast(65537) == 18446462603027742720U);
		assert(defyx_reciprocal_fast(15000001) == 10316166306300415204U);
		assert(defyx_reciprocal_fast(3845182035) == 10302264209224146340U);
		assert(defyx_reciprocal_fast(0xffffffff) == 9223372039002259456U);
	});

	runTest("Dataset initialization (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), []() {
		initCache("test key 000");
		uint64_t datasetItem[8];
		defyx::initDatasetItem(cache, (uint8_t*)&datasetItem, 0);
		assert(datasetItem[0] == 0x680588a85ae222db);
		defyx::initDatasetItem(cache, (uint8_t*)&datasetItem, 10000000);
		assert(datasetItem[0] == 0x7943a1f6186ffb72);
		defyx::initDatasetItem(cache, (uint8_t*)&datasetItem, 20000000);
		assert(datasetItem[0] == 0x9035244d718095e1);
		defyx::initDatasetItem(cache, (uint8_t*)&datasetItem, 30000000);
		assert(datasetItem[0] == 0x145a5091f7853099);
	});

	runTest("Dataset initialization (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), []() {
		initCache("test key 000");
	...
	defyx::NativeRegisterFile reg;
	defyx::BytecodeMachine decoder;
	defyx::InstructionByteCode ibc;
	...
	runTest("IADD_RS (decode)", RANDOMX_FREQ_IADD_RS > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IADD_RS - 1;
		...
		assert(ibc.type == defyx::InstructionType::IADD_RS);
	...
	runTest("IADD_RS with immediate (decode)", RANDOMX_FREQ_IADD_RS > 0, [&] {
		...
		instr.opcode = defyx::ceil_IADD_RS - 1;
		instr.mod = 8;
		instr.dst = registerHigh | defyx::RegisterNeedsDisplacement;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IADD_RS);
		assert(ibc.idst == &reg.r[defyx::RegisterNeedsDisplacement]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.shift == 2);
	...
	runTest("IADD_RS with immediate (decode)", RANDOMX_FREQ_IADD_RS > 0, [&] {
		reg.r[defyx::RegisterNeedsDisplacement] = 0x8000000000000000;
		reg.r[registerSrc] = 0x2000000000000000;
		decoder.executeInstruction(ibc, pc, nullptr, config);
		assert(reg.r[defyx::RegisterNeedsDisplacement] == imm64);
	});

	runTest("IADD_M (decode)", RANDOMX_FREQ_IADD_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IADD_M - 1;
		instr.mod = 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IADD_M);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL1Mask);
	});

	runTest("ISUB_R (decode)", RANDOMX_FREQ_ISUB_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_ISUB_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::ISUB_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("ISUB_M (decode)", RANDOMX_FREQ_ISUB_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_ISUB_M - 1;
		instr.mod = 0;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::ISUB_M);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL2Mask);
	});

	runTest("IMUL_R (decode)", RANDOMX_FREQ_IMUL_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IMUL_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IMUL_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("IMUL_M (decode)", RANDOMX_FREQ_IMUL_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IMUL_M - 1;
		instr.mod = 0;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerDst;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IMUL_M);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(*ibc.isrc == 0);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL3Mask);
	});

	runTest("IMULH_R (decode)", RANDOMX_FREQ_IMULH_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IMULH_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IMULH_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("IMULH_M (decode)", RANDOMX_FREQ_IMULH_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IMULH_M - 1;
		instr.mod = 0;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IMULH_M);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL2Mask);
	...
	runTest("ISMULH_R (decode)", RANDOMX_FREQ_ISMULH_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_ISMULH_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::ISMULH_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("ISMULH_M (decode)", RANDOMX_FREQ_ISMULH_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_ISMULH_M - 1;
		instr.mod = 3;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::ISMULH_M);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL1Mask);
	...
	runTest("IMUL_RCP (decode)", RANDOMX_FREQ_IMUL_RCP > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IMUL_RCP - 1;
		instr.dst = registerHigh | registerDst;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IMUL_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &ibc.imm);
		assert(ibc.imm == defyx_reciprocal(imm32));
	});
	...
	runTest("IMUL_RCP zero imm32 (decode)", RANDOMX_FREQ_IMUL_RCP > 0, [&] {
		...
		instr.setImm32(0);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::NOP);
	});

	runTest("INEG_R (decode)", RANDOMX_FREQ_INEG_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_INEG_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::INEG_R);
		assert(ibc.idst == &reg.r[registerDst]);
	});
	...
	runTest("IXOR_R (decode)", RANDOMX_FREQ_IXOR_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IXOR_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IXOR_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("IXOR_M (decode)", RANDOMX_FREQ_IXOR_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IXOR_M - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerDst;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IXOR_M);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(*ibc.isrc == 0);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL3Mask);
	...
	runTest("IROR_R (decode)", RANDOMX_FREQ_IROR_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IROR_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IROR_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("IROL_R (decode)", RANDOMX_FREQ_IROL_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_IROL_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::IROL_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("ISWAP_R (decode)", RANDOMX_FREQ_ISWAP_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_ISWAP_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::ISWAP_R);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
	});
	...
	runTest("FSWAP_R (decode)", RANDOMX_FREQ_FSWAP_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FSWAP_R - 1;
		instr.dst = registerHigh | registerDst;
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FSWAP_R);
	...
	runTest("FADD_R (decode)", RANDOMX_FREQ_FADD_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FADD_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FADD_R);
		assert(ibc.fdst == &reg.f[registerDst]);
	...
	runTest("FADD_M (decode)", RANDOMX_FREQ_FADD_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FADD_M - 1;
		instr.mod = 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FADD_M);
		assert(ibc.fdst == &reg.f[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL1Mask);
	...
	runTest("FSUB_R (decode)", RANDOMX_FREQ_FSUB_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FSUB_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FSUB_R);
		assert(ibc.fdst == &reg.f[registerDst]);
		assert(ibc.fsrc == &reg.a[registerSrc]);
	});
	...
	runTest("FSUB_M (decode)", RANDOMX_FREQ_FSUB_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FSUB_M - 1;
		instr.mod = 2;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FSUB_M);
		assert(ibc.fdst == &reg.f[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL1Mask);
	...
	runTest("FSCAL_R (decode)", RANDOMX_FREQ_FSCAL_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FSCAL_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FSCAL_R);
		assert(ibc.fdst == &reg.f[registerDst]);
	});
	...
	runTest("FMUL_R (decode)", RANDOMX_FREQ_FMUL_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FMUL_R - 1;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FMUL_R);
	...
	runTest("FDIV_M (decode)", RANDOMX_FREQ_FDIV_M > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FDIV_M - 1;
		instr.mod = 3;
		instr.dst = registerHigh | registerDst;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FDIV_M);
		assert(ibc.fdst == &reg.e[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL1Mask);
	...
	runTest("FSQRT_R (decode)", RANDOMX_FREQ_FSQRT_R > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_FSQRT_R - 1;
		instr.dst = registerHigh | registerDst;
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::FSQRT_R);
		assert(ibc.fdst == &reg.e[registerDst]);
	});
	...
	runTest("CBRANCH (decode) 100", RANDOMX_FREQ_CBRANCH > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_CBRANCH - 1;
		instr.dst = registerHigh | registerDst;
		instr.setImm32(imm32);
		instr.mod = 48;
		decoder.compileInstruction(instr, 100, ibc);
		assert(ibc.type == defyx::InstructionType::CBRANCH);
		assert(ibc.idst == &reg.r[registerDst]);
	...
	runTest("CFROUND (decode)", RANDOMX_FREQ_CFROUND > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_CFROUND - 1;
		instr.src = registerHigh | registerSrc;
		instr.setImm32(imm32);
		decoder.compileInstruction(instr, 100, ibc);
		assert(ibc.type == defyx::InstructionType::CFROUND);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == 18);
	});
	...
	runTest("ISTORE L1 (decode)", RANDOMX_FREQ_ISTORE > 0, [&] {
		defyx::Instruction instr;
		instr.opcode = defyx::ceil_ISTORE - 1;
		instr.src = registerHigh | registerSrc;
		instr.dst = registerHigh | registerDst;
		instr.setImm32(imm32);
		instr.mod = 1;
		decoder.compileInstruction(instr, pc, ibc);
		assert(ibc.type == defyx::InstructionType::ISTORE);
		assert(ibc.idst == &reg.r[registerDst]);
		assert(ibc.isrc == &reg.r[registerSrc]);
		assert(ibc.imm == imm64);
		assert(ibc.memMask == defyx::ScratchpadL1Mask);
...
#ifdef __OpenBSD__
	vm = defyx_create_vm(RANDOMX_FLAG_DEFAULT | RANDOMX_FLAG_SECURE, cache, nullptr);
#else
	vm = defyx_create_vm(RANDOMX_FLAG_DEFAULT, cache, nullptr);
#endif
	...
	runTest("Hash test 1a (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_a);

	runTest("Hash test 1b (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_b);

	runTest("Hash test 1c (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_c);

	runTest("Hash test 1d (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_d);

	runTest("Hash test 1e (interpreter)", stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_e);

	if (RANDOMX_HAVE_COMPILER) {
		defyx_release_cache(cache);
		defyx_destroy_vm(vm);
		vm = nullptr;
		cache = defyx_alloc_cache(RANDOMX_FLAG_JIT);
		initCache("test key 000");
#ifdef __OpenBSD__
		vm = defyx_create_vm(RANDOMX_FLAG_DEFAULT | RANDOMX_FLAG_SECURE, cache, nullptr);
#else
	...
	runTest("Hash test 2a (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_a);

	runTest("Hash test 2b (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_b);

	runTest("Hash test 2c (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_c);

	runTest("Hash test 2d (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_d);

	runTest("Hash test 2e (compiler)", RANDOMX_HAVE_COMPILER && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), test_e);

	defyx_destroy_vm(vm);
	vm = nullptr;

	auto flags = defyx_get_flags();

	defyx_release_cache(cache);
	cache = defyx_alloc_cache(RANDOMX_FLAG_ARGON2_SSSE3);

	runTest("Cache initialization: SSSE3", (flags & RANDOMX_FLAG_ARGON2_SSSE3) && RANDOMX_ARGON_ITERATIONS == 3 && RANDOMX_ARGON_LANES == 1 && RANDOMX_ARGON_MEMORY == 262144 && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), []() {
		initCache("test key 000");
		uint64_t* cacheMemory = (uint64_t*)cache->memory;
		assert(cacheMemory[0] == 0x191e0e1d23c02186);
		assert(cacheMemory[1568413] == 0xf1b62fe6210bf8b1);
	...
	if (cache != nullptr)
		defyx_release_cache(cache);
	cache = defyx_alloc_cache(RANDOMX_FLAG_ARGON2_AVX2);

	runTest("Cache initialization: AVX2", (flags & RANDOMX_FLAG_ARGON2_AVX2) && RANDOMX_ARGON_ITERATIONS == 3 && RANDOMX_ARGON_LANES == 1 && RANDOMX_ARGON_MEMORY == 262144 && stringsEqual(RANDOMX_ARGON_SALT, "DefyX\x03"), []() {
		initCache("test key 000");
		uint64_t* cacheMemory = (uint64_t*)cache->memory;
		assert(cacheMemory[0] == 0x191e0e1d23c02186);
		assert(cacheMemory[1568413] == 0xf1b62fe6210bf8b1);
