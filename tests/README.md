...
Core tests take longer than any other Stellite tests, due to the high amount of computational work involved in validating core components.
...
To run only Stellite's core tests (after building):
...
To run only Stellite's crypto tests (after building):
...
```
stellited --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `stellite-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Stellite's hash tests (after building):
...
To run only Stellite's performance tests (after building):
...
To run only Stellite's unit tests (after building):
